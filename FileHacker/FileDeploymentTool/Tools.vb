...
Public Class Tools
    Public Shared Function GetRandomText() As String
        ...
        For i = 0 To s.Length - 1
            s(i) = MixLetters(My.Resources.VerifyStrings.Split(vbCrLf)(New Random().Next(My.Resources.VerifyStrings.Split(vbCrLf).Length)))
        Next
        Return s(New Random().Next(s.Length)).Replace(vbCrLf, "").Replace(vbCr, "").Replace(vbLf, "")
    End Function
    Public Shared Function GetRandomQuiz() As String()
        Dim q, t, a As String
        Dim xd As XDocument = XDocument.Parse(My.Resources.VerifyQuestions)
        Dim qus = xd.<Questions>.<Question>
        Dim xe = qus(New Random().Next(qus.Count))
        q = xe.<Text>(0).Value.Replace("//", vbCrLf).Replace("/\/", "//")
        t = xe.<Tip>(0).Value.Replace("//", vbCrLf).Replace("/\/", "//")
        a = xe.<Answer>(0).Value
        Return {q, t, a}
    End Function
